...
[![Version](https://img.shields.io/badge/version-0.42.0-blue.svg)](install.sh)
...
│   ├── harness-lessons.md               <-- one-line harness corrections, 400w budget (from aidex)
...
| `harness-lessons.md` | One-line corrections about how the *harness* behaves — Bash, the tools, hooks, shell quoting, skill loading — that hold in every project. Capped at 400 words, because it is always-on; a lesson needing a paragraph belongs in a skill's `references/`, and one already enforced by a check does not belong at all |
...
| **`aidex-coverage`** | Model-invocable | The one testing canon for a Django + DRF + Vue + Vitest + Playwright stack: which layer a behaviour belongs in, the test shapes per layer, isolated E2E infrastructure and the `test-e2e.sh` generator (reads `.context/testing-profile.md`), and which tests to run for a change — the full suite is a boundary gate, never a per-phase one. Deliberately does **not** run audits or touch `module-map.json` / `coverage-matrix.json` — that split belongs to `aidex-audit`'s `test-coverage` playbook. |
