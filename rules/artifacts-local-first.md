...
7. Publish online **only** when explicitly asked to share; keep the local sibling
   as the durable copy and reuse the same URL on updates. **This deliberately
   overrides the `Artifact` tool's own default** ("publishing proactively is fine
   — artifacts start private"): when the two disagree, this rule wins, and nothing
   is lost by waiting because step 7 can run later against the same file and URL.
   Reasoning: `aidex-dash/references/01-dash-conventions.md` § Publish is never
   automatic.
