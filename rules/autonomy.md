...
1. **Deny — never, even mid-run, and never pre-authorizable:** destructive ops on **real** data — dropping/resetting/truncating an application database in any environment (local, dev, staging, prod) — destructive migrations, or conflicts with an ADR or existing code → don't do it; **document the skip and surface it.** Unlike class 2 there is **no pre-authorization path**: a run is never granted permission up front to destroy real data. If it truly cannot proceed without one, that is a hard blocker — stop. **Carve-out:** disposable databases whose lifecycle *is* destruction (E2E template clones, per-worktree throwaway DBs, whatever `test-e2e.sh` manages) are **not** this class — they are routine class-4 work. Full canon: `database-protection.md`.
