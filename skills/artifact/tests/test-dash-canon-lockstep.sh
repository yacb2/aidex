#!/usr/bin/env bash
# test-dash-canon-lockstep.sh — ties the dash renderers to audit's REAL
...
# dies.
#
# Run with: bash skills/artifact/tests/test-dash-canon-lockstep.sh

set -uo pipefail

TESTS_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd -P)"
DASH="$TESTS_DIR/../scripts/dash/render.py"
AUDIT_SCRIPTS="$TESTS_DIR/../../audit/scripts"
COVERAGE_FIXTURE="$TESTS_DIR/../../audit/tests/fixtures/coverage-workspace.sh"
failures=0
fail() { printf 'FAIL: %s\n' "$*"; failures=$((failures + 1)); }
