#!/usr/bin/env bash
# allowed-tools lockstep guard for artifact.
#
...
#
# Run with: bash skills/artifact/tests/test-allowed-tools-lockstep.sh
...
if [[ "$failures" -eq 0 ]]; then
  echo "OK — artifact allowed-tools in lockstep with its body: $DECLARED"
  exit 0
fi
exit 1
