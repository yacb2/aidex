...
|---|---|
| plan | did `aidex-plan` produce a plan the user accepted, or was it re-triaged, re-scoped or rewritten by hand |
| execute | did `aidex-plan-exec` run the phases start to finish; where did it stop to ask, and was the question one the plan should have settled |
| close | did the run reach close-out (`close-plan.sh`), and were deferrals registered or left as prose |

Rules:

- **`worklist-advance.sh` is shared.** It lives in `aidex-conventions` and the backlog
