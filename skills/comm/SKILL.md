...
**Read `${CLAUDE_PLUGIN_ROOT}/skills/conventions/references/communication-conventions.md`
...
|---|---|---|
| Work this side has agreed to do | a backlog entry | `bash ${CLAUDE_PLUGIN_ROOT}/skills/backlog/scripts/register-item.sh --origin communication --communication <folder> --title "<item>"` |
...
```bash
python3 ${CLAUDE_PLUGIN_ROOT}/skills/conventions/scripts/validate.py --type communications
```
...
- **request** — for a stakeholder/product *requirement* to act on (the ask), not the raw message; log the message here, capture the requirement there.
- **conventions** — parent convention for `.context/communications/`.
