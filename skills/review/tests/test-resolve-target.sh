...
# 2026-08-11: `--partition skills` reported audit at 3,428 LOC where the same
# path as a target reads 4,748, and SEVEN markdown-only skills (bugfix,
# decision, loop, request, research, skill,
# workflow -- 14 files, 1,503 LOC) did not appear at all. A caller following the
...
# .md is not a source extension, so pointing the resolver at skills/review
...
PTR_LINE="$(grep -n 'Read `${CLAUDE_PLUGIN_ROOT}/skills/review/references/01-review-angles.md`' "$SKILL_MD" | head -1 | cut -d: -f1)"
...
# Field-found 2026-08-23 reviewing skills/audit/scripts/coverage: EXCLUDE_DIRS
...
if [ -f "$README" ]; then
  ROW="$(grep -F '| **`review`**' "$README" | head -1)"
  if [ -z "$ROW" ]; then
    fail "the README has no review row — the public description of this skill vanished"
  else
    case "$ROW" in
      *refus*|*Refus*)
        case "$ROW" in
          *--partition*) : ;;
          *) fail "the README says review refuses an oversize target but never names --partition — that is the inverted claim 27d0ce5 made false and 4d8e97d corrected" ;;
        esac ;;
      *) : ;;
    esac
  fi
