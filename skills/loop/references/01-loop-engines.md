...
The decision and discipline layer for `loop`. The skill does not run loops;
...
The **Workflow** engine is the same machinery as plan-exec's durable Workflow forms (`skills/plan-exec/assets/workflows/*.workflow.js`) reused as a loop engine — NOT a parallel forms system; build a concrete loop asset only against a real loop case (per 'no abstractions for single-use code').
