...
The `.context/loops/` artifact written by `loop`. Follows the shared
...
- A loop-spec is **execution discipline**, not a plan. If the work is multi-phase
  but not a loop, use `plan`. A plan may *spawn* a loop-spec for one phase.
- The engine choice may deserve its own ADR (`decision`) only if it sets a
