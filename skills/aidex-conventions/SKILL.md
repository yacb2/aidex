...
description: Creates and structures project knowledge under .context/ — plans (single-file or modular YYYYMMDD-feature/00-index.md + phase files), decisions/ADRs, requests, research, references, docs, issues, backlog entries, and CLAUDE.md. MUST trigger when the user says any of — "create a plan" / "make a plan for" / "let's plan X" / "I need to plan the migration of X" / "draft a plan" / "plan out the X feature" → plans; "ADR" / "decision record" / "we decided" / "document this decision" / "we're deciding between X and Y" / "we keep flip-flopping on X" → decisions; "stakeholder asked for" / "the client requested" / "X asked us to add Y" / "capture this as a request" / "PM passed us these requirements" → requests; "research how X works" / "investigate X" / "spike on X" / "I need to investigate Y before deciding" → research; "create a reference for X" / "document how X works" / "document how the X system is configured" / "write up the X architecture" → references; "library docs for X" / "document the X SDK" → docs; "add this to the backlog" / "track this for later" → suggest backlog-register; "catalog bugs" / "list gaps and opportunities" / "audit the state of X" → suggest audit. Activates PROACTIVELY whenever the user describes a new task, feature request, technical/architectural decision, stakeholder ask, or area to investigate — propose the right .context/ artifact BEFORE implementing. Multi-phase or multi-layer work (backend + frontend + tests) → use modular plan format (.context/plans/YYYYMMDD-feature/ with 00-index.md + phase files). Do NOT use for: auditing the Claude Code ecosystem (skills, symlinks, MEMORY.md, CLAUDE.md health) → use aidex; operating on existing audits (scaffold, validate, escalate, migrate) → use audit; creating individual backlog entries → use backlog-register.
