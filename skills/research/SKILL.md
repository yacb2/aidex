...
the `conventions` canon, but research's own body shape is defined here.

## Workflow

1. Read the shared documentation canon:
   `${CLAUDE_PLUGIN_ROOT}/skills/conventions/references/00-global.md` (language,
...
4. Body = research's own minimal shape, three sections:
   ...
   work should iterate against), link out to an `loop` loop-spec rather than
...
```bash
python3 ${CLAUDE_PLUGIN_ROOT}/skills/conventions/scripts/validate.py --type research
```
...
- **conventions** — owns the shared documentation canon (`00-global.md`:
