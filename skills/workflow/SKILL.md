...
the durable machinery already exists in plan-exec's Workflow forms
...
plan · loop = repeat one task until a machine check passes · **workflow = author
...
1. **Fan-out suitability (step 0).** Is the work genuinely **decomposable** into
   ...
   If it is really the *sequential phases of a written plan* → hand to `plan-exec`.
   If it is *repeat one task until a check passes* → hand to `loop`. If it is a
...
6. **Autonomy surface (step 1.5).** Resolve the permission borders so the run is
   ...
   [`../conventions/references/autonomy-conventions.md`](../conventions/references/autonomy-conventions.md).)
...
3. **Launch via the `Workflow` tool — do not rebuild durability.** Reuse the
   plan-exec forms keyed off the shape:
...
|---|---|
| Execute a written multi-phase plan (there is a plan doc) | `plan-exec` |
| Repeat one task until a machine check passes (tests/typecheck/build green) | `loop` |
| Plan multi-step work (no orchestration yet) | `plan` |
...
- **plan-exec** — owns the durable `Workflow` forms (pipeline / fan-out / review)
  and the durability CORE this skill delegates to. A plan-exec batch run *is* a Workflow;
  workflow authors a **standalone** one (no plan doc) with explicit per-agent models.
- **loop** — the sibling designer for repeat-until-green loops; same designer
  pattern (survey → spec → delegate), different execution shape.
- **conventions** — owns the shared `.context/` canon, the work-list, the autonomy
