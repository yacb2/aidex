#!/usr/bin/env bash
# Shape-enum lockstep guard for workflow.
#
# loop has had test-engine-enum-lockstep.sh since the 2026-07-12 engine
# drift (an engine existed in the decision matrix but not in the front-matter
# enum). workflow declares its shape list in FOUR places and had no such
...
#
# Run with: bash skills/workflow/tests/test-shape-enum-lockstep.sh
