...
#
# The first three checks already existed as separate scripts, so knowing whether the backlog was
...
#   definition open items missing a contract field — they never enter a sweep (define-check.py)
#
...
. "$SCRIPT_DIR/../../conventions/scripts/_lib.sh" 2>/dev/null || {
  echo "ERROR: cannot source conventions/scripts/_lib.sh from $SCRIPT_DIR" >&2
  exit 1
}
...
printf '%d of 4 checks found something actionable:\n' "${#PROBLEMS[@]}"
