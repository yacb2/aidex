...
#                                  mid-run by plan-exec: a single-file plan's
...
#   --surface <internal|behaviour|ui|ops>  what the change is visible as (default: internal;
#                                  ops = no test surface: config, infrastructure, canon, other-repo state);
...
# injected, which a last-write-wins parser honours. Found 2026-08-10 by review.
yaml_escape() {
  printf '%s' "$1" \
    | tr '\n\r\t' '   ' \
    | sed -e 's/\\/\\\\/g' -e 's/"/\\"/g'
...
# both the directory order and the awk match. Found 2026-08-10 by review.
#
...
# digits), which is the case this filter exists for. Found 2026-08-10 by review.
next_backlog_id() {
...
# close-item.sh already used the insert-if-missing idiom. Found 2026-08-10 by review.
...
# Acceptance comment. Found 2026-08-10 by review.
#
...
emit_backlog_stub() {
  ...
  # P0 item landed in the active queue as `open · M`. Found 2026-08-10 by review.
...
# --- dispatcher: strip leading "backlog" if present ---
if [[ "${1:-}" == "backlog" ]]; then shift; fi
...
regen_index() {
  ...
  {
    printf '# Backlog\n\n'
    printf '_Auto-generated by `backlog` from front-matter. Do not edit by hand._\n\n'
    printf '**Updated:** %s\n' "$today"
    printf '**Active:** %d · **Blocked:** %d · **Doing:** %d · **Awaiting owner:** %d\n\n' "$active_count" "${#SEC_BLOCKED[@]}" "$doing_count" "${#SEC_AWAITING[@]}"
    ...
    [[ ${#SEC_AWAITING[@]} -gt 0 ]] && emit_section "Awaiting owner" "${SEC_AWAITING[@]}"
...
if [[ $LIST_ONLY -eq 1 ]]; then
  ...
  declare -a P0=() P1=() P2=() P3=() PUNK=() BLOCKED=() AWAITING=()
  for f in "$BACKLOG_DIR"/*.md; do
    ...
    awaiting="$(read_field awaiting "$f")"
    line="$(printf '  - %s %s(%s)%s\n      %s%s%s' \
  ...
  # safe under `set -u` on bash 3.2. Found 2026-08-10 by review.
  ...
  print_section "Awaiting owner (parked by close-item --sweep)" "$C_DIM" ${AWAITING[@]+"${AWAITING[@]}"}
  print_section "Unclassified (legacy — preview: migrate-priorities.sh, write: --apply)" "$C_RED" ${PUNK[@]+"${PUNK[@]}"}

  total=$((${#P0[@]} + ${#P1[@]} + ${#P2[@]} + ${#P3[@]} + ${#PUNK[@]} + ${#BLOCKED[@]} + ${#AWAITING[@]}))
  [[ $total -eq 0 ]] && printf '\n  (no open entries)\n'
  exit 0
fi
...
case "$SURFACE" in internal|behaviour|ui|ops) ;; *) die "invalid surface: $SURFACE (internal|behaviour|ui|ops)" ;; esac
...
# path; only the ref derivation is branch-specific. Found 2026-08-10 by review.
ORIGIN_REF=""
...
case "$ORIGIN" in
  ...
  communication)
    ...
    ;;
  plan)
    # plan-exec defers emergent work mid-run (BL-220): the run keeps going and the
...
if [[ -n "$ESCALATE_TO" ]]; then
  ...
  # fail later. Found 2026-08-10 by review, reproduced 2026-08-10.
  #
