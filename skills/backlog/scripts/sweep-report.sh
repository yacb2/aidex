...
WRAP="$(cd "$SCRIPT_DIR/../../artifact/scripts" 2>/dev/null && pwd -P || true)/wrap-report.sh"
HTML="${OUT%.md}.html"
...
print(profile_language(find_context_dir(sys.argv[2])) or "en")' "$DASH_PY" "$(dirname "$HTML")" 2>/dev/null || echo en)"
  ...
  echo "NOTE: artifact is not installed next to this skill, so no page was written beside $OUT." >&2
fi
