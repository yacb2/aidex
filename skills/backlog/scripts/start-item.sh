...
#
# Why this exists (BL-134). Across 103 eligible bug items, `bugfix` fired
# on 3. It was not losing to silence (only 4 items ran with no skill at all): it
# was losing the race to `backlog`, first on 23. Its trigger describes a
...
is not optional. Follow bugfix (skills/bugfix/SKILL.md):
...
Exception: purely visual/CSS bugs - bugfix, "Exception" section.
--------------------------------------------------------------------
ROUTE
elif [[ -z "$TYPE" ]]; then
