...
#
# Run with: bash skills/backlog/tests/test-estimate-calibration.sh

set -uo pipefail

TESTS_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd -P)"
CAL="$TESTS_DIR/../scripts/estimate-calibration.py"
FIXTURE="$TESTS_DIR/../../audit/tests/fixtures/usage-retro-corpus.sh"
MINER="$TESTS_DIR/../../audit/scripts/usage-retro/mine_items.py"
failures=0
fail() { printf 'FAIL: %s\n' "$*"; failures=$((failures + 1)); }
