...
The `.context/worktrees/00-index.md` artifact written by `worktree`'s
`bootstrap`. Follows the shared `.context/` canon
(`conventions/references/00-global.md`); only the worktree-specific rules are
declared here.
...
updated: YYYY-MM-DD
version: 1.0.0
worktree_new:  "bash ${CLAUDE_PLUGIN_ROOT}/skills/worktree/scripts/worktree.sh new <slug> --branch <branch>"
worktree_up:   "bash ${CLAUDE_PLUGIN_ROOT}/skills/worktree/scripts/worktree.sh up <slug>"
worktree_down: "bash ${CLAUDE_PLUGIN_ROOT}/skills/worktree/scripts/worktree.sh down <slug>"
worktree_list: "bash ${CLAUDE_PLUGIN_ROOT}/skills/worktree/scripts/worktree.sh list --porcelain"
```

- The `worktree_*` fields are what executors read (plan-exec Orient,
...
- **Open questions** is not a scratchpad for a run's in-flight doubts. At teardown, the
  ...
  only questions still genuinely open. `plan-exec` does this symmetrically with its
...
- **Consumed by `plan` / `plan-exec` / `loop`'s Isolation step** —
  each of those calls `worktree` for the `worktree.sh new` invocation to use
...
- **Never hand-copied between projects.** Each project's facts (topology, infra
  ...
  on the surface.
- **`conventions/references/worktree-conventions.md`** holds the shared
