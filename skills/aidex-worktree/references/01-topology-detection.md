...
## Placement, and why isolating the stack does not isolate the session

Isolation is defined over **database, ports and containers** — the three the interview
sizes. Placement decides where the worktree directory lands, and it is the one thing
here that touches the *agent's* environment rather than the stack's.
...
ports and containers are perfectly isolated. Database, ports and containers being right
says nothing about the session's own settings.
