...
def stale_report(axes: list[Axis], owners, root: Path, refs: Path) -> list[str]:
    ...
    Modelled on audit's coverage sweep -- the high-signal event there is
...
def render_matrix(report: dict) -> str:
    lines = ["<!-- GENERATED by reference/scripts/docs-census.py — do not hand-edit -->",
...
def main() -> int:
    ...
    if not axes:
        ...
        print("\nCreate one from "
              "skills/reference/assets/templates/00-profile.md.template",
              file=sys.stderr)
        return 2
