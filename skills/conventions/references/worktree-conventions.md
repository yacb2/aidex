...
how much of the environment to isolate**. Owned here; referenced by `plan`,
`plan-exec`, and `loop`. Like the autonomy canon this is a *behavioral*
...
> Detection and the per-project bootstrap interview are owned by the `worktree`
> skill, not by this canon or by the consuming skills directly — this file documents
> the shared reasoning (what a worktree isolates, the opt-in gate); `worktree` is where the
...
`worktree`'s `worktree.sh new <slug> --branch <branch>`; the project supplies only
...
> `worktree/references/01-topology-detection.md`.
...
front-matter fields of `.context/worktrees/00-index.md` (written by `worktree
bootstrap`) and surfaced by `detect-project-commands.sh` as
`worktree_up_command`/`worktree_down_command` — the same way `plan-exec` detects the
...
`worktree`'s Axis 4) includes: "images built for the worktree removed (`--rmi
...
- The orphan sweep (`worktree/scripts/orphan-sweep.sh`, wired into
...
`worktree bootstrap` first if the project has no `.context/worktrees/00-index.md`
yet), instead of reasoning about isolation inline:

- **plan** — at plan Orient, record `worktree.sh new <slug> --branch <branch>`
...
- **plan-exec** — at **Orient (phase 0)** run the recorded command; with no
  worktree setup in the project, `EnterWorktree` and note it. Tear down at completion.
- **loop** — the design interview pre-declares the isolation next to the autonomy
  ...
  elsewhere).
- **audit** — usually no worktree (read-mostly). Exception: a security audit doing
