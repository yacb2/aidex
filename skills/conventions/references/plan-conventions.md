...
Two modes, chosen by a triage step that runs at the top of **every** `plan`
...
- **research** — the *how* is unknown; planning now is invention. Route to `research`.
...
- Exactly **one phase**. A scoped plan is therefore never batch-eligible, and
  `plan` does not offer `plan-exec` for it — the existing ≥2-phase rule
  already covers that.
...
`.context/plans/00-index.md` is the **auto-generated roll-up of state across all plans** (active grouped by `## Doing` / `## Open`, closed rolled up from `_archive/`) — the plans analogue of `backlog/00-index.md`. Do **not** hand-edit it; the `plan` skill regenerates it (`scripts/reindex-plans.sh`) on plan creation and on `close-plan.sh`.
...
> consider spawning a loop-spec via `loop` and handing off execution rather
...
A phase may declare the optional fields below that the executor (`plan-exec`) reads to drive batch execution. **All of them share one carrier rule:**
...
- **`tier: mechanical | standard | hard`** — how hard the phase's work is. The executor maps it to a model and an effort level. Omitted = `standard`. Tier also sets detail depth (see Philosophy): mechanical phases may carry prescriptive steps/code; hard phases get contracts + gates only.
  ...
  **This table is the mapping's only home.** `plan-exec`, its references and its
...
- **`phase-type: hitl-align | afk-impl`** — the execution mode:
  ...
  - **`hitl-align`** — a phase that requires human-in-the-loop alignment (defining scope, success criteria, or a design concept; see `plan` Step 0). It is **never** promoted into a Workflow — the executor runs it interactively. Defining a phase's own spec/gate is the judgment an agent grading its own questions gets wrong, so it stays human-gated.
- **`tests: unit | api | component | e2e | none`** — `afk-impl`-only: which test layer's acceptance test closes this phase (API or E2E, whichever describes what the phase promises; unit tests still run in parallel regardless). Write that one acceptance test up front and leave it red — see `plan` Step 3 and `plan-exec`'s batch gate. A list is allowed when more than one layer genuinely closes the phase. `none` requires a written reason: `tests: none  # reason: <why no acceptance test applies>`. `hitl-align` phases are exempt, same as the gate rule.
