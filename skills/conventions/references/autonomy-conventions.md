...
ask the user**. Owned here; referenced by `loop`, `plan`,
`plan-exec`, and `audit`. This is a *behavioral* canon, not a
...
- Plan → the `plan` design + the `plan-exec` Orient step.
- Audit → the `/aidex:audit new` kickoff (scope + borders).
- Loop → the `loop` design interview (Step 1.5).
...
- **plan** — the design is the front-loading home: capture the **autonomy
...
- **plan-exec** — resolve all clarifications at **Orient** (phase 0), then run
