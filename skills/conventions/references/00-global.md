...
In `backlog/`, the `00-index.md` is **auto-regenerated** from front-matter — never hand-edit. See [`01-backlog-conventions.md`](../../backlog/references/01-backlog-conventions.md).
...
- `loops/` — move `done`/`dropped` loop-specs on close (see `loop`'s `02-loop-spec-conventions.md`).
...
- **Who sets it:** the skills that already produce or verify work fill it in as
  a byproduct — e.g. `bugfix` (the GREEN test output), `plan-exec`
  (per-phase commit SHAs / verification artifacts), `audit` (a `proof:`
...
`workflow`); the rest are project-local. If a project uses them, document
...
artifacts) and **`audit`'s `validate-audit.sh`** (`.context/audits/`
...
installed `${CLAUDE_PLUGIN_ROOT}/skills/…` it pointed at `~/.aidex/.context/`, which does not exist, and
