...
Shared canon. Consumers: `plan-exec` (§2, between phases) and `backlog`'s
...
|---|---|---|
| `plan-exec` | a phase | the plan's `00-index.md` |
| `backlog sweep` | ~5 items, or any cluster boundary | the work-list (`## Checkpoints`) and the sweep report |
...
`${CLAUDE_PLUGIN_ROOT}/skills/conventions/scripts/resolve-review-scope.sh --files working-diff`
...
`${CLAUDE_PLUGIN_ROOT}/skills/conventions/references/review-scope-conventions.md` before picking
...
diff (`plan-exec/references/03-deferring-emergent-work.md`). A deferral must outlive
