...
> one live invocation).
>
> **All three are diff-anchored**, which is why `review`
...
belongs to `review`, which measures the target first (`resolve-review-target.sh`)
...
- Route a **module-as-it-stands** review to `review`, not to a scope in the enum
