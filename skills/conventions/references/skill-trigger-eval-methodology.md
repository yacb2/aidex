...
| Inventory **mislabel cleanup** | **EXHAUSTED — small** | Run-4: exhaustive audit of all 40 `should_trigger=true` queries found **exactly one** genuine mislabel (backlog #7, read/list in a create-skill). One in forty ⇒ the ~35% is *not* inventory pollution. |
| **Structural split of mega-skills** | **REFUTED (two fair isolated pilots, 2026-05-16, B-β + B-γ)** | **B-β** `plan` (colliding intent, whole aidex family neutralized): adj recall **20%**. **B-γ** `decision` (no-affordance-twin intent, family **incl. plan** neutralized): adj recall **40%** — vs same-session same-env control `backlog` 78%. Both far below the usable single-purpose floor; structure perfected precision (→100% both) but never recall. Mechanism, **directional only** (n=20/pilot — one matcher coin-flip ≈ ±10pp; do **not** cite the pp figures as measured point estimates): the no-twin intent out-recalling the colliding one (40 vs 20) is consistent with **two components** — a *partial* plan-create affordance collision (the twin-specific part) plus a larger *twin-independent* residual suppressor that survives split + full isolation even with no native twin. Not the aidex siblings, not structure. See `.context/audits/2026-05-15-skill-trigger-eval/stage-a-b-assessment.md` "ISOLATED RE-TEST (B-β)" + "CONFOUND-CONTROL PILOT (B-γ)". **Do not re-run or extend (no request/research pilots)** — settled across a colliding and a non-colliding intent. |
...
| Arm | plan adjusted recall | False positives |
|---|---|---|
...
|---|---|---|---|
| `loop` | 12/15, median **1766 s** | 13, 13, 12 /15 @ 101/114/104 s | **~17x** |
| `comm` | 14/20, median **1314 s** | 19/20 twice @ ~150 s | ~9x |
...
noise and not timeout truncation: `comm` q1/q5/q7/q9 were PTY-FAIL x3 stable
against detector-PASS x3 stable. Magnitude ranges from ~7% (`loop`, one
divergent query) to ~25% (`comm`, 19/20 vs 14/20), and is **zero** on others
(`decision` 20/20, `conventions` 10/10). You cannot predict which
...
B-β `plan` (colliding intent) 20% adj recall with the whole aidex family
neutralized; B-γ `decision` (no-affordance-twin intent, family incl.
plan neutralized) 40% adj recall — both vs 78% same-env control. The
...
investigation (no request/research pilots) is warranted.
...
config. Control `backlog` 88.9% (frozen band [60%,90%] →
valid). Sibling adj recall: plan **20%** (byte-identical to isolated
B-β — the colliding plan-create ceiling is family-state-invariant),
decision 50% (within one coin-flip of B-γ 40%, not an improvement),
request 30%, research 60%, reference 50%, skill 70%;
...
predicate path**: `decision` measured **5 TP** in the run-6 session
...
> **Status (2026-05-18, resolved for `decision` only — bounded).** The
> single-variable ES-strip A/B on `decision` (30 paired obs/arm, McNemar
...
**The regression this catches and the marker cannot.** Ask `comm` to log a
...
A negative or cannibalization probe aimed at `plan-exec` (or any executor) must
...
`coverage` has **9 positive queries**. Comparing the
...
positive control on `coverage` query 06 answered, unprompted, that aidex "has no
...
modular sibling descriptions (`decision`, `-reference`, `-request`, `-research`,
...
| check | result |
|---|---|
| a description anchoring on a **sibling's name** before its `Not for:` clause (the defect behind the `loop` collision) | **none** |
...
against each other but not against `comm` or `skill`, while those two name all
