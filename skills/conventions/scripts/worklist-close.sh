...
# section is a class-(b) record, not a queue the run owes, and audit calls this
...
[[ "$open_refs" -gt 0 ]] && echo "note: $open_refs tracked upstream ref(s) — run backlog/scripts/reconcile.sh for closure propagation" >&2
