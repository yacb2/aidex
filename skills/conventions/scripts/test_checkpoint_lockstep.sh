...
#
#   1. plan-exec/SKILL.md   (§2, between phases)
#   2. backlog/SKILL.md     (sweep run mode, every ~5 items / cluster boundary)
#
...
#
# Run with: bash skills/conventions/scripts/test_checkpoint_lockstep.sh

set -uo pipefail

SKILLS="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd -P)"
CANON="${CANON_OVERRIDE:-$SKILLS/conventions/references/checkpoint-conventions.md}"
EXEC="${EXEC_OVERRIDE:-$SKILLS/plan-exec/SKILL.md}"
SWEEP="${SWEEP_OVERRIDE:-$SKILLS/backlog/SKILL.md}"
...
case "$EXEC_FLAT" in *checkpoint-conventions.md*) ;; *) err "plan-exec/SKILL.md does not reference checkpoint-conventions.md — the consumer is free to drift" ;; esac
...
[ -n "$sec2" ] || err "plan-exec/SKILL.md has no '### 2.' checkpoint section to check"
restated="$(printf '%s\n' "$sec2" | grep -cE '^[0-9]+\. \*\*(Code-review|Commit|Defer|Context check)' || true)"
[ "${restated:-0}" -lt 2 ] || err "plan-exec/SKILL.md §2 restates $restated of the four moves as its own numbered list — point at the canon instead"

# ---------- cell 3: the sweep consumer points at the same file and names its cadence ----------
case "$SWEEP_FLAT" in *checkpoint-conventions.md*) ;; *) err "backlog/SKILL.md (sweep run mode) does not reference checkpoint-conventions.md" ;; esac
case "$SWEEP_FLAT" in *"5 items"*) ;; *) err "backlog/SKILL.md does not name the sweep checkpoint cadence (~5 items)" ;; esac
case "$SWEEP_FLAT" in *"exit code"*) ;; *) err "backlog/SKILL.md does not name what the sweep seed adds (what ran, with which exit codes)" ;; esac

# ---------- and the canon is reachable from the hub ----------
grep -q "references/checkpoint-conventions.md" "$SKILLS/conventions/SKILL.md" \
  || err "checkpoint-conventions.md is not routed from conventions/SKILL.md — unreachable canon"
