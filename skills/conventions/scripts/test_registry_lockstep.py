...
and has no standing to judge (BL-115).
  ...
  7b. every skill that fans out — by DECLARING Workflow/Agent in allowed-tools, or by
     ...
     Workflow script passes it vacuously — which is how review came to spawn 34
     agents that all inherited the session's model by omission. The declaration trigger
     had the same hole one level out: audit mandated a Workflow fan-out and an
...
test fails loudly. Run with:
    ...
    python3 skills/conventions/scripts/test_registry_lockstep.py
"""
...
def _model_policy_failures(rel: str, head: str, body: str) -> list[str]:
    ...
    # any mandate that straddles a newline — how the artifact Skill mandate hid (BL-080).
    ...
    if re.search(r"\bTask\b", tools.group(1)):
        failures.append(f"{rel} declares 'Task' in allowed-tools; this suite names the "
                        f"subagent launcher 'Agent' (review/SKILL.md) — one name, "
...
# The orchestrator directory is the bare namespace root. Every other skill is a
...
def _name_prefix_failures(pairs: list[tuple[str, str]]) -> list[str]:
    ...
    skill-conventions.md — the invocation segment of a plugin skill is its
    DIRECTORY name (verified at runtime against the docs' claim that `name:`
    overrides it), so the directory and `name:` must be the same short,
    unprefixed token: `skills/plan/` + `name: plan` -> `/aidex:plan`. The hub
    is the bare namespace root (`skills/aidex/` + `name: aidex`). A directory
    still carrying the `aidex-` prefix would invoke as `/aidex:aidex-plan`, and
    a directory renamed without its `name:` (or the reverse) is half a rename.
    """
    out = []
    for d, name in pairs:
        if d != NAMESPACE_ROOT and d.startswith(NAMESPACE_ROOT + "-"):
            out.append(f"skills/{d}/ still carries the '{NAMESPACE_ROOT}-' prefix — the "
                       f"plugin namespace already supplies it; the directory is "
                       f"'{d[len(NAMESPACE_ROOT) + 1:]}'")
            continue
        if not name:
            out.append(f"skills/{d}/SKILL.md declares no `name:`")
        elif name != d:
            out.append(f"skills/{d}/SKILL.md declares name '{name}' — a plugin skill's "
                       f"invocation segment is its directory, so `name:` is '{d}'")
    return out
...
def main() -> int:
    ...
    # review spawned 34 agents that way, every one of them inheriting the session's
    ...
    mig_path = SKILLS_DIR / "conventions" / "scripts" / "migrate-conventions.py"
    ...
    if not mig_txt:
        ...
        if not block:
            ...
            ref_canon = (SKILLS_DIR / "conventions" / "references"
            ...
            for n in sorted(names):
                ...
                if n not in ref_canon and n not in (
                        SKILLS_DIR / "conventions" / "references"
    ...
    lib = SKILLS_DIR / "audit" / "scripts" / "_lib.sh"
    ...
    if not m_types:
        ...
        enum_sites = {
            "audit-conventions.md": SKILLS_DIR / "conventions" / "references" / "audit-conventions.md",
            "audit/references/04-playbooks.md": SKILLS_DIR / "audit" / "references" / "04-playbooks.md",
            "audit/SKILL.md": SKILLS_DIR / "audit" / "SKILL.md",
        }
        for site_name, site in enum_sites.items():
            ...
            tmpl = (SKILLS_DIR / "audit" / "assets" / "templates"
    ...
    # Two shapes: the live plugin form `aidex:<skill>` and the pre-plugin
    # `aidex-<skill>`, which must now resolve to nothing so a stale reference is
    # still caught. Without the `:` branch the guard goes hollow — every migrated
    # reference would collapse to the bare hub name and never be checked.
    name_re = re.compile(r"\baidex(?::[a-z0-9][a-z0-9-]*|(?:-[a-z0-9]+)+)?\b")
    ...
    def _referenced_names(text: str) -> set[str]:
        ...
        if fm:
            ...
            if desc:
                found |= {_resolve(t) for t in name_re.findall(_unquote(desc.group(2)))}
        for sec in section_re.findall(text):
            found |= {_resolve(t) for t in name_re.findall(sec)}
        return found
    ...
    # (retired) `hooks/aidex-router.sh` was checked here for emitting a skill that
    # exists. The hook is unwired and no longer installed (its replay came out
    # p=0.72), so its phrase table is frozen history in the pre-plugin spelling —
    # judging it against the live tree reports a rename that is not a defect.
    ...
    for probe, label in (
        ([(f"{NAMESPACE_ROOT}-audit", f"{NAMESPACE_ROOT}-audit")],
         "a directory still carrying the prefix"),
        ([(f"{NAMESPACE_ROOT}-audit", "audit")],
         "a half-renamed directory whose `name:` is already short"),
        ([("audit", "audits")], "a directory and `name:` that disagree"),
        ([("audit", "")], "a SKILL.md with no `name:`"),
    ):
        if not _name_prefix_failures(probe):
            failures.append(f"the name-prefix guard passed {label}")
    if _name_prefix_failures([(NAMESPACE_ROOT, NAMESPACE_ROOT), ("audit", "audit")]):
