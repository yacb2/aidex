...
MIGRATE_COMMS_SH = (Path(__file__).resolve().parents[2]
                    / "comm" / "scripts" / "migrate-communications.sh")
...
# scaffolded on demand (worklists by the worklist scripts, workflows by workflow);
...
# backlog's --escalate-to handshake). Format-checked, never existence-checked.
...
# Pre-00-* uppercase board names still parse everywhere (audit's
...
# artifact's `LANG_FIELD` (scripts/dash/wrap_report.py) on purpose: that script is
...
def check_filename(type_name: str, path: Path) -> Finding | None:
    ...
    if type_name == "audits":
        ...
        # (aligned with audit's validate-audit.sh / test-canonical-filenames.sh).
...
def check_body_language(type_name: str, path: Path, text: str,
    ...
    through backlog/scripts/normalize-language.sh (BL-226), which filters this
...
def check_audit_folders(context_dir: Path) -> list[Finding]:
    """Folder-level audit checks, aligned with audit's validate-audit.sh
...
def check_plan_phase_gates(path: Path, text: str, fm: dict | None) -> list[Finding]:
    ...
    Only afk-impl phases are batch-eligible (the plan-exec promotion threshold);
...
def check_plan_tests_field(path: Path, text: str, fm: dict | None) -> list[Finding]:
    ...
    front and left red (plan / plan-exec, Phase 7)."""
    findings: list[Finding] = []
    if "_archive" in path.parts:
        return findings
...
def check_plan_scoped_shape(path: Path, text: str, fm: dict | None) -> list[Finding]:
    ...
    # stray **Verify:** in the body. Found by reviewing this function with review.
