...
#   3. plan-exec/SKILL.md          — must delegate to the reference, so the
...
#
# Run with: bash skills/conventions/scripts/test_review_scope_lockstep.sh

set -uo pipefail

SKILLS="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd -P)"
SCRIPT="$SKILLS/conventions/scripts/resolve-review-scope.sh"
REF="$SKILLS/conventions/references/review-scope-conventions.md"
EXEC="$SKILLS/plan-exec/SKILL.md"
...
case "$flat" in
  ...
  *) err "plan-exec/SKILL.md does not reference review-scope-conventions.md — the consumer is free to drift" ;;
esac
case "$flat" in
  *resolve-review-scope.sh*) ;;
  *) err "plan-exec/SKILL.md does not name resolve-review-scope.sh — the checkpoint has no resolver to call" ;;
esac
...
if [ -n "$named" ]; then
  ...
  [ -f "$SKILLS/conventions/scripts/$base" ] \
    || err "plan-exec names '$named' but no such script exists under conventions/scripts/"
  case "$named" in
    '${CLAUDE_PLUGIN_ROOT}/skills/conventions/scripts/'*) ;;
...
CONV_SKILL="$SKILLS/conventions/SKILL.md"
for f in "$SKILLS"/conventions/references/*.md; do
  b=$(basename "$f")
  grep -q "references/$b" "$CONV_SKILL" \
    || err "references/$b is not routed from conventions/SKILL.md — unreachable canon"
done
