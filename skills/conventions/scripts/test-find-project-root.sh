...
#
# Run with: bash skills/conventions/scripts/test-find-project-root.sh
...
# nine worktree scripts source this same _lib.sh.
...
if [[ -d "$SKILLS_DIR" ]]; then
  ...
  while IFS= read -r hit; do
    [[ "$hit" == */conventions/scripts/_lib.sh ]] && continue
    extra="$extra $hit"
