...
1. Read the decision conventions canon:
   ...
   (Decisions section; or `.claude/skills/conventions/references/request-decision-conventions.md`
...
```bash
bash ${CLAUDE_PLUGIN_ROOT}/skills/conventions/scripts/close-dated-artifact.sh decisions <slug> --status superseded --superseded-by decision/<new-adr>
bash ${CLAUDE_PLUGIN_ROOT}/skills/conventions/scripts/close-dated-artifact.sh decisions <slug> --status dropped
```
...
```bash
python3 ${CLAUDE_PLUGIN_ROOT}/skills/conventions/scripts/validate.py --type decisions
```
