...
BACKLOG_REINDEX="$AIDEX_DIR/skills/backlog/scripts/register-item.sh"
if [[ -x "$BACKLOG_REINDEX" ]]; then
  ...
  note "backlog not installed at $AIDEX_DIR — skipped backlog index seed"
fi

PLAN_REINDEX="$AIDEX_DIR/skills/plan/scripts/reindex-plans.sh"
if [[ -x "$PLAN_REINDEX" ]]; then
  ( cd "$PROJECT_DIR" && "$PLAN_REINDEX" --reindex ) >/dev/null 2>&1
  note "plans index seeded via $PLAN_REINDEX --reindex"
else
  note "plan not installed at $AIDEX_DIR — skipped plans index seed"
fi
...
DETECTOR="$AIDEX_DIR/skills/conventions/scripts/detect-project-commands.sh"
