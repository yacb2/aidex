...
The Python validator at `${CLAUDE_PLUGIN_ROOT}/skills/aidex-conventions/scripts/validate.py`
...
- `${CLAUDE_PLUGIN_ROOT}/skills/aidex-conventions/references/reference-conventions.md` (refs + docs)
- `${CLAUDE_PLUGIN_ROOT}/skills/aidex-conventions/references/plan-conventions.md` (plans)
...
Fast implementation hint: shell out to `${CLAUDE_PLUGIN_ROOT}/skills/aidex-audit/scripts/validate-audit.sh --json <audits-dir>` when available — it produces the same violations in JSON. Parse and map to check codes above. Fall back to manual checks only if the script is missing.
...
- **[AG] Anti-patterns**:
  ...
  - Empty directories — apply this decision matrix (per `${CLAUDE_PLUGIN_ROOT}/skills/aidex-conventions/references/claudemd-conventions.md` § Project Context Directory):
...
- **[AJ] Scratch bucket** (the one check that looks at the project root, not `.context/`): ephemeral output has exactly one home, `_tmp/` at the project/workspace root (per `${CLAUDE_PLUGIN_ROOT}/skills/aidex-conventions/references/claudemd-conventions.md` § Scratch Output).
