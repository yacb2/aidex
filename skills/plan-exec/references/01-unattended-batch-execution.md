...
- The forms ship as versioned assets, all embedding the single-sourced durability CORE
  (see [`../conventions/references/workflow-core.md`](../../conventions/references/workflow-core.md)):
...
([plan-conventions.md](../../conventions/references/plan-conventions.md) §"Optional phase metadata"):
...
layer of its acceptance test (unit/api/component/e2e), written up front per `plan`'s
...
`conventions/references/plan-conventions.md` §"Optional phase metadata" and is not
...
branch; **escalate the blocked phase to the backlog** (`backlog` if installed) with the
