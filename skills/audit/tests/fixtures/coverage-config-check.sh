...
# ---------------------------------------------------------------------------
P="$WS/own-checker"
mkpy "$P/skills/audit/scripts/coverage/config_check.py" <<'EOF'
# marker: this directory is a checkout of the checker itself
EOF
mkpy "$P/skills/audit/tests/test-coverage-config-check.sh" <<'EOF'
# asserts the checker flags `-n auto`
EOF
mkpy "$P/skills/audit/tests/fixtures/coverage-config-check.sh" <<'EOF'
ENV PYTEST_ADDOPTS="-n auto"
EOF
