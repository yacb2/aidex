...
D="$TX/-Users-user-Documents-projects-demo-ws"
mkdir -p "$D"
...
#     (a read, not a run), and a subagent transcript that runs validate.py. ---
{
  ...
  py_bash b2 "bash skills/backlog/scripts/close-item.sh --sweep BL-777"
...
mkdir -p "$D/s6/subagents"
{
  ...
  py_bash b4 "python3 skills/conventions/scripts/validate.py --type plans"
  py_bash_ok b4 "OK"
...
# --- s7: the user names the eta page and the session wraps it ---
{
  ...
  py_bash b5 "bash \${CLAUDE_PLUGIN_ROOT}/skills/artifact/scripts/wrap-report.sh --in _tmp/eta.md --out .context/reports/2026-01-07-eta.html"
