...
#
# Run with: bash skills/audit/tests/test-extract-provenance.sh
...
# The typed prompt is followed by plan, THEN a machine body with its own
...
# extract.py hardcoded `-Users-user-...` in BOTH of its project helpers:
...
ALT="$(mktemp -d)"
cp -R "$TX"/-Users-user-Documents-projects-demo-ws \
      "$ALT/-Users-alice-Documents-projects-demo-ws"
cp -R "$TX"/-Users-user-Documents-projects-demo-ws "$ALT/-Users-alice--claude"
cp -R "$TX"/-Users-user-Documents-projects-demo-ws "$ALT/-Users-alice-.claude"
...
SYNTH="$(mktemp -d)/-Users-user-Documents-projects-demo-ws"
mkdir -p "$SYNTH"
{
...
SCRATCH="$(mktemp -d)"
cp -R "$TX"/-Users-user-Documents-projects-demo-ws \
      "$SCRATCH/-Users-user-Documents-projects-demo-ws"
# a session scratchpad, as Claude Code encodes it
cp -R "$TX"/-Users-user-Documents-projects-demo-ws \
      "$SCRATCH/-private-tmp-claude-501--Users-user-Documents-projects-demo-ws-abc123-scratchpad"
# the project's own _tmp, per the scratch-output convention
cp -R "$TX"/-Users-user-Documents-projects-demo-ws \
      "$SCRATCH/-Users-user-Documents-projects-demo-ws--tmp"
