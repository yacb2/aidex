...
CPD="$CPTX/-Users-user-Documents-projects-aa-ws"
mkdir -p "$CPD"
{
...
CPD2="$CPTX/-Users-user-Documents-projects-zz-ws"
mkdir -p "$CPD2"
...
DIRTY="$(mktemp -d)"
DD="$DIRTY/-Users-user-Documents-projects-demo-ws"
mkdir -p "$DD"
{
  py_dirty_prompt() {
...
#     strip the literal `-Users-user-Documents-projects-`, so on any
...
XD="$XT/-Users-user-Documents-projects-mix-ws"
mkdir -p "$XD"
xprompt() {
...
# ---------------------------------------------------------------------------
SLOWTX="$(mktemp -d)/-Users-user-Documents-projects-slow-ws"
mkdir -p "$SLOWTX"
{
...
cat = by["cat skills/backlog/scripts/close-item.sh"]
assert cat["read_only"] and cat["exit_code"] == 0, "cat of a path is a read"
runs = [e for e in ev if e["command"].startswith("bash skills/backlog")]
