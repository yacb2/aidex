...
# unresolved findings, grouped by severity, ready for loop to execute
...
# --- scaffold through loop (it owns id allocation + canon filename) ---
NEWLOOP=""
for candidate in \
...
[[ -n "$NEWLOOP" ]] || die "loop script not found. Run './install.sh --update' to install it."

info "Emitting remediation loop-spec for $RUN_SLUG ($COUNT unresolved findings)"
LOOP_FILE="$("$NEWLOOP" new "$SLUG")"
[[ -n "$LOOP_FILE" && -f "$LOOP_FILE" ]] || die "loop did not return a valid loop-spec path"
