...
{
  printf '# Audits\n\n'
  printf '_Auto-generated by `audit` from run front-matter + `00-inventory.md`. Do not edit by hand._\n\n'
  printf '**Updated:** %s\n' "$TODAY"
  if [[ ${#UNREC[@]} -gt 0 ]]; then
  ...
  if [[ ${#UNREC[@]} -gt 0 ]]; then
    ...
    printf '_Audit folders with no `index.md`/`00-index.md` run header — not counted above. Add a run index (see `audit` templates) to track them here:_\n\n'
