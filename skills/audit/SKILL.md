...
See [audit-conventions](../conventions/references/audit-conventions.md) for the full convention.
...
**Read `${CLAUDE_PLUGIN_ROOT}/skills/audit/references/04-playbooks.md` before choosing a type.** It holds what each
...
`${CLAUDE_PLUGIN_ROOT}/skills/conventions/references/worklist-conventions.md` **first** — it holds
...
3. **Read `${CLAUDE_PLUGIN_ROOT}/skills/audit/references/02-id-conventions.md`** before writing the first
...
6. **Read `${CLAUDE_PLUGIN_ROOT}/skills/audit/references/03-lifecycle.md`** before setting any finding's
...
> `${CLAUDE_PLUGIN_ROOT}/skills/conventions/references/worktree-conventions.md` for the
...
**Read `${CLAUDE_PLUGIN_ROOT}/skills/audit/references/05-migration-guide.md` before accepting any move.**
...
`plan-exec` (at plan close) and `bugfix` (at GREEN) surface a one-line
...
Quick summary. **Read `${CLAUDE_PLUGIN_ROOT}/skills/audit/references/01-principles.md` before
...
`conventions/references/audit-conventions.md` § Core principles.
...
- **conventions** — defines the audit convention itself
- **backlog** — handles the other side of escalation (`/aidex:backlog --origin audit --finding <id>`)
- **artifact** — renders inventory boards and the coverage matrix as interactive HTML on demand (`render.sh audit <methodology>` / `render.sh coverage`); publishing stays user-gated
