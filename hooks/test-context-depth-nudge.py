...
for depth, expect in [(199_999, "SILENT"), (200_000, "1.4x slower"),
                      ...
                      (299_999, "stops paying for itself"), (300_000, "handoff is due"),
                      (900_000, "handoff is due")]:
    sid = f"TEST-b{depth}"
    clean(sid)
...
check("band 3 still escalates", "handoff is due" in fire(p3, sid), "")
...
try:
    ...
    check("instructs the model not to self-fire", "do not hand off on your own" in txt, txt[:90])
except Exception as e:
